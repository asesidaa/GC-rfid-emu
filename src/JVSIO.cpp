...
namespace {
	...
	uint8_t getCommandSize(uint8_t* command, uint8_t len) {
		auto cmd = static_cast<JVSIO::Cmd>(*command);
		switch (cmd) {
		case JVSIO::Cmd::kCmdReset:
		case JVSIO::Cmd::kCmdAddressSet:
			return 2;
		case JVSIO::Cmd::kCmdIoId:
		case JVSIO::Cmd::kCmdCommandRev:
		case JVSIO::Cmd::kCmdJvRev:
		case JVSIO::Cmd::kCmdProtocolVer:
		case JVSIO::Cmd::kCmdFunctionCheck:
			return 1;
		case JVSIO::Cmd::kCmdMainId:
			break;  // handled later
		case JVSIO::Cmd::kCmdSwInput:
			return 3;
		case JVSIO::Cmd::kCmdCoinInput:
		case JVSIO::Cmd::kCmdAnalogInput:
		case JVSIO::Cmd::kCmdRotaryInput:
			return 2;
		case JVSIO::Cmd::kCmdKeyCodeInput:
			return 1;
		case JVSIO::Cmd::kCmdScreenPositionInput:
			return 2;
		case JVSIO::Cmd::kCmdRetry:
			return 1;
		...
		case JVSIO::Cmd::kCmdDriverOutput:
			return command[1] + 2;
		case JVSIO::Cmd::kCmdAnalogOutput:
			return command[1] * 2 + 2;
		case JVSIO::Cmd::kCmdCharacterOutput:
			return command[1] + 2;
...
uint8_t* JVSIO::getNextCommand(uint8_t* len, uint8_t* node) {
	for (;;) {
		...
		if (!command_size) {
			Serial.println("Command size is 0");
		...
		auto cmd = static_cast<JVSIO::Cmd>(rx_data_[rx_read_ptr_]);
		switch (cmd) {
		case JVSIO::Cmd::kCmdReset:
			Serial.println("kCmdReset");
			senseNotReady();
			for (uint8_t i = 0; i < nodes_; ++i)
				address_[i] = kBroadcastAddress;
			rx_available_ = false;
		...
		case JVSIO::Cmd::kCmdAddressSet:
			...
			break;
		case JVSIO::Cmd::kCmdCommandRev:
			Serial.println("kCmdCommandRev");
			pushReport(JVSIO::kReportOk);
			pushReport(0x13);
			break;
		case JVSIO::Cmd::kCmdJvRev:
			Serial.println("kCmdJvRev");
			pushReport(JVSIO::kReportOk);
			pushReport(0x30);
			break;
		case JVSIO::Cmd::kCmdProtocolVer:
			Serial.println("kCmdProtocolVer");
			pushReport(JVSIO::kReportOk);
			pushReport(0x10);
			break;
		case JVSIO::Cmd::kCmdMainId:
			Serial.println("kCmdMainId");
			...
			break;
		case JVSIO::Cmd::kCmdRetry:
			Serial.println("kCmdRetry");
			sendStatus();
			break;
		case JVSIO::Cmd::kCmdDriverOutput:
		case Cmd::kCmdDriverInput:
		case JVSIO::Cmd::kCmdIoId:
		case JVSIO::Cmd::kCmdFunctionCheck:
		case JVSIO::Cmd::kCmdSwInput:
		case JVSIO::Cmd::kCmdCoinInput:
		...
		case JVSIO::Cmd::kCmdCharacterOutput:
		case JVSIO::Cmd::kCmdCoinAdd:
		case Cmd::kCmdTaito01:
		case Cmd::kCmdTaito03:
		case Cmd::kCmdTaito04:
		case Cmd::kCmdTaito05:
		case Cmd::kCmdHandlePayout:
			*len = command_size;
			rx_read_ptr_ += command_size;
			return &rx_data_[rx_read_ptr_ - command_size];
		default:
...
void JVSIO::boot() {
	...
	for (;;) {
		...
		tx_data_[1] = 3;  // Bytes
		tx_data_[2] = static_cast<uint8_t>(JVSIO::Cmd::kCmdReset);
		...
		tx_data_[0] = kBroadcastAddress;
		tx_data_[1] = 3;  // Bytes
		tx_data_[2] = static_cast<uint8_t>(JVSIO::Cmd::kCmdAddressSet);
	...
	while (!rx_available_ && data_->available() > 0) {
		...
		if (rx_size_ >= 2 && ((rx_data_[1] + 2) == rx_size_)) {
			...
			if ((rx_data_[0] == kBroadcastAddress &&
				rx_data_[2] == static_cast<uint8_t>(JVSIO::Cmd::kCmdReset)) ||
				matchAddress(rx_data_[0], address_, nodes_)) {
				// Broadcast or for this device.
